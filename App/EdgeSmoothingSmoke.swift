...
@MainActor enum EdgeSmoothingSmoke {
    static func run(content:DrivingContent,renderer:SceneRenderer,output:URL) throws -> [String:Any] {
        ...
        for preset:DrivingCameraPreset in [.chase,.side3,.driver] {
            var rig=DrivingCameraRig()
            for _ in 0..<200 { renderer.camera=try rig.view(preset:preset,body:pose.body,bonnetPosition:content.bonnetPosition,driverPosition:content.driverPosition,yaw:content.simulation.visualSnapshot.body.orientation.z,trackHeading:0) { try content.simulation.road.geometry.height(at:$0,startingAt:0) } }
            ...
            let name=preset.rawValue.lowercased().replacingOccurrences(of:" ",with:"-")
            for mode in 0..<2 {
                let label=mode==0 ? "off":"on",sorted=times[mode].sorted()
                try save(frames[mode],"smoothing-\(name)-\(label)",width,height)
                modes.append(["view":name,"smoothing":label,"samples":60,"gpuMedianMS":sorted[30],"gpuP95MS":sorted[57]])
            }
            guard frames[0] != frames[1] else { throw RendererError.unavailable("Edge smoothing did not change \(name) view") }
            ...
            if preset == .driver {
                ...
                try save(odd,"smoothing-driver-odd-size",961,641)
                ...
                guard restored==frames[1] else { throw RendererError.unavailable("Edge smoothing resize did not restore the same image") }
            }
        }
        print("MSAA_VISUAL views=3 repeats=\(repeatCount) resizeRestore=1 modes=\(modes)")
        return ["sampleCount":renderer.rasterSampleCount,"memoryless":renderer.usesMemorylessMultisampling,"imagesRGBA_SHA256":images,"modes":modes,"repeatCount":repeatCount,"resizeRestoreExact":true,"note":"960x640 stationary single-car scene; interleaved GPU command samples, not gameplay FPS. Odd-size driver image is 961x641."]
    }
}
