...
| Rendering | Functional | Modern forward+ PBR path only: linear HDR, Hillaire atmosphere with a drifting cloud layer and an overcast day, four-cascade shadows with contact shadows, GTAO at a quarter on the Air preset, SSR with temporal reuse, motion blur, bloom, spatial MetalFX upscaling with a deadline-aware dynamic resolution controller, generated road/terrain/grass/tree geometry with view-frustum culling, pit garages, tyre walls, the native starting grid painted, 26 generated material sets and original art uploaded block-compressed with cached encodes, lens-lit brake/head lights, rear-view mirror, tyre smoke, dust, skid marks, wet track, rain with drops on the windscreen, sun glare, heat haze, a lens for the television view; 31 original camera presets and TV/fly directors kept as presentation logic. Prebuilt shader library and a pipeline archive so nothing compiles during a race; per-pass GPU timer; pipelined and 60 Hz-paced measurement loops. Driver's-eye view 9.2 ms at native (6.8 ms busy, 93–96% of frames on time paced), full session 214 MB. Classic fixed-function path, its graphics oracles and its 18 test files retired 2026-09-24; see RENDERER_REPLACEMENT.md |
| Car lights | Functional | Brake and light commands drive an emissive lens channel on the car geometry; original point-billboard kernels and their reference tests retired with the classic path; see RENDERER_REPLACEMENT.md |
| Fly camera / scene height | Partial | F10 exposed with saved zoom; original motion reference-tested; height comes from the generated road geometry rather than the original scene-graph traversal, which is retired; other LODs pending |
...
| Race weekend / session sequence | Partial | RaceWeekend qualifies one driver at a time as the original does, refuses out-of-turn runs, and builds a grid from the ranking; New Session offers the grid order and a race starts its entries in grid order; a finished race shows a full classification; the window still runs only the human's qualifying session |
| Qualifying ranking / starting order | Parity verified | Original qualifying insertion and the drivers-list/last-race/reversed starting order ported; ten ranking cases match a verbatim excerpt of the newly pinned raceresults.cpp, including millisecond rounding, no-time entries and ties; not yet wired to a session sequence; see QUALIFYING_ORDER.md |
| Racing in the driving window | Functional | Race runtime behind the window with one published frame for both session kinds, the whole field drawn, live classification with gaps and penalties, and Race in New Session with a car count; the window's data path is captured headlessly and inspected, the SwiftUI layout is not; see RACE_WINDOW.md |
| Field rendering | Functional | Renderer draws a whole field with 17 instances per car, an explicit viewer and mirror contents carrying every other car; three cars on the original grid slots take draws 146→246 and triangles 277,247→497,247 with 5.75% of the frame changed and an identical in-process repeat; no per-car livery; see FIELD_RENDERING.md |
| Native BT opponent handling | Parity verified | Semantic port of cardata.cpp, opponent.cpp and the opponent-aware driver.cpp paths; three cars over 20,000 ticks match all 22,176 raw control values exactly with opponents classified on 4,914 callbacks; solo baselines unchanged; team behaviour ported but unexercised; see BT_TRAFFIC.md |
| Authoritative native race runtime | Functional | Original ReOneStep order over a native field: clock, 0.02 s scheduling for BT and human entries, multi-car physics, pit management, timing, gaps, complete rules and sorting, with race results; three cars complete two laps in 124,468 ticks and repeat byte-identically; callback cadence and countdown length match the oracle; no trajectory parity claimed and BT still has no opponent handling; see RACE_RUNTIME.md |
| Native race rules / penalties | Parity verified | Semantic ReRaceRules port; corner-cut time penalty matches over 200 steps at five speeds, lap-time elimination and human exemption match, and eight-step pit-lane and stop-and-go walks match rule state, penalty queue, kind and lap-to-clear at every step; authored positions only, not yet connected to a running race; see RACE_RULES.md |
| Native starting grid | Parity verified | Semantic initStartingGrid port; 15 configurations (1–16 cars, 1–4 rows, both pole sides, deep grids) match the original exactly across 770 values; grid-placed four-car fields match all 142 published fields over 1,200 clean and 1,500 contacting ticks with zero difference; rolling starts rejected; see STARTING_GRID.md |
| Heterogeneous multi-car field | Functional | One definition per car with per-car collision boxes; three variants place, keep their definitions and repeat 255,600 values exactly; no original counterpart because the reference harness loads one car model per world |
