...
| Rendering | Functional | Modern forward+ PBR path only: linear HDR, atmosphere, four-cascade shadows with contact shadows, GTAO, SSR, motion blur, bloom, spatial MetalFX upscaling with dynamic resolution, generated road/terrain/grass/tree geometry, pit garages, tyre walls on the corners, the native starting grid painted, and materials, lens-lit brake/head lights, rear-view mirror, tyre smoke, dust and skid marks from the published wheel skid, visual-only wet-track and rain conditions with puddles, spray and streaks, dithered detail switches, swaying tree shadows, sun glare, heat haze; 31 original camera presets and TV/fly directors kept as presentation logic. The seven section-24 signposts in place; prebuilt shader library shipped in the app, scalers pre-warmed per resolution step, memory under the 1.5 GB budget (650 MB full session). Classic fixed-function path, its graphics oracles and its 18 test files retired 2026-09-24; see RENDERER_REPLACEMENT.md |
