...
| Rendering | Functional | Modern forward+ PBR path only: linear HDR, atmosphere, four-cascade shadows with contact shadows, GTAO, SSR, motion blur, bloom, spatial MetalFX upscaling with dynamic resolution, generated road/terrain/grass/tree geometry and materials, lens-lit brake/head lights, rear-view mirror, tyre smoke and dust from the published wheel skid; 31 original camera presets and TV/fly directors kept as presentation logic. Classic fixed-function path, its graphics oracles and its 18 test files retired 2026-09-24; see RENDERER_REPLACEMENT.md |
